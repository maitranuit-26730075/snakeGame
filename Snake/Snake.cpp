...
void showGameOver(const string& reason) {
	gotoxy(12, 8);
	cout << "====================";
...
int main() {
	...
	while (!exitGame) {
		...
		while (!gameOver) {
			...
			if (snake.hitWall()) {
				gameOver = true;
				gameOverReason = "Ban da dung vao tuong!";
				break;
			}
			...
			if (snake.hitSelf()) {
				gameOver = true;
				gameOverReason = "Ban da can vao than!";
				break;
			}
		...
		// Cho nguoi choi chon choi lai hoac thoat
		while (true) {
			char choice = static_cast<char>(_getch());

			// Nhan R de reset va choi lai
			if (choice == 'r' || choice == 'R') {
				break;
			}

			// Nhan Q de thoat game
